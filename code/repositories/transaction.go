...
func (t *transactionRepository) FindPaginatedByCustomerID(ctx context.Context, customerID uint64, params dtos.PaginationParams) ([]models.Transaction, int64, error) {
	var transactions []models.Transaction
	var total int64

	// Buat query dasar
	query := t.db.WithContext(ctx).Model(&models.Transaction{}).Where("customer_id = ?", customerID)
	countQuery := t.db.WithContext(ctx).Model(&models.Transaction{}).Where("customer_id = ?", customerID)
...
func (t *transactionRepository) CreateTransaction(ctx context.Context, tx models.Transaction) error {
